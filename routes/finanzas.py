...
def api_invitados_detalle():
    ...
    # Invitados con su patrocinador (miembro que los trajo)
    rows = con.execute("""
        SELECT
    ...
    # Miembros disponibles para asignar como patrocinador
...
def asignar_patrocinador():
    """Asigna o cambia el miembro patrocinador de un invitado."""
    if not _es_admin_o_financiero():
        return jsonify({"ok": False, "error": "Sin permiso"}), 403
    data = request.get_json(silent=True) or {}
    ...
    _registrar_auditoria(con, session["uid"], inv_id,
        "asignar_patrocinador", "invitado_de", str(row["invitado_de"]), str(pat_id))
    con.commit()
    return jsonify({"ok": True, "msg": "Relación actualizada"})
