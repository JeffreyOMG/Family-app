    ...
    if q:
        ...
        if tipo in ("todo","usuarios"):
            for u in con.execute(
                "SELECT nombre,usuario,rol,foto FROM usuarios WHERE nombre LIKE ? OR usuario LIKE ?",
                (pat,pat)
            ).fetchall():
        ...
        if tipo in ("todo","publicaciones"):
            for p in con.execute(
                "SELECT p.texto,u.nombre,p.fecha FROM publicaciones p JOIN usuarios u ON u.id=p.usuario_id WHERE p.texto LIKE ?",
                (pat,)
            ).fetchall():
        ...
        if tipo in ("todo","noticias"):
            for n in con.execute(
                "SELECT titulo,contenido,fecha FROM noticias WHERE titulo LIKE ? OR contenido LIKE ?",
                (pat,pat)
            ).fetchall():
                resultados.append({"titulo":n["titulo"],"descripcion":n["contenido"][:100],"tipo":"Noticia","fecha":n["fecha"][:10]})
        if tipo in ("todo","eventos"):
            for e in con.execute(
                "SELECT e.titulo,e.descripcion,e.fecha_evento,e.tipo,u.nombre FROM eventos e JOIN usuarios u ON u.id=e.usuario_id WHERE e.titulo LIKE ? OR e.descripcion LIKE ?",
                (pat,pat)
            ).fetchall():
