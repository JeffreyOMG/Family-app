"""
Cajitas de Ahorro Familiar — rutas para unirse a una cajita.
Las rutas de crear/aportar siguen en finanzas.py.
"""
import secrets
...
def _asegurar_codigo(con, cajita_id):
    ...
    row = con.execute(
        "SELECT codigo FROM cajitas_ahorro_codigos WHERE cajita_id=%s", (cajita_id,)
    ).fetchone()
    if row:
        return row["codigo"]
    for _ in range(10):
...
def get_codigo(cid):
    ...
    es_miembro = con.execute(
        "SELECT 1 FROM cajita_miembros WHERE cajita_id=%s AND usuario_id=%s", (cid, _uid())
    ).fetchone()
...
def unirse():
    ...
    con = get_db()
    row = con.execute(
        "SELECT cajita_id FROM cajitas_ahorro_codigos WHERE codigo=%s", (codigo,)
    ).fetchone()
    if not row:
    ...
    con.commit()
    cajita = con.execute(
        "SELECT id, nombre, descripcion FROM cajitas_ahorro WHERE id=%s", (cajita_id,)
    ).fetchone()
